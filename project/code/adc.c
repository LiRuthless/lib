...
int32 get_track_error(void)
{
	int32 diff = 0;  // 外侧电感差（带符号）
	int32 left_diff = 0;  // 外侧电感差（带符号）
    int32 right_diff = 0;  // 内侧电感差（带符号）
    int32 denominator = 0;
	
    left_diff  = adc_filted[0] - adc_filted[3];  // 外侧电感差（带符号）
    right_diff = adc_filted[1] - adc_filted[2];  // 内侧电感差（带符号）
	...
	diff = ( 1000 * (left_diff + right_diff) / denominator );

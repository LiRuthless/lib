...
{
	if(adc_filted[1] > 250 && adc_filted[2] < 50)
	{
...
{
	if(adc_filted[2] > 250 && adc_filted[1] < 50)
	{
		kernel_state = KERNEL_REISLAND;
	}
}
...
//	if((adc_filted[0] > 1500 && adc_filted[3] > 2500) && angle_y < 5 && round_flag == 0)
	if((adc_filted[1] + adc_filted[2] < 200) && round_flag == 0)

...
ADRC_Controller adrc_L = {0.0f, 0.0f, 8.0f, 188.4f, 8874.0f, 31.4f};
ADRC_Controller adrc_R = {0.0f, 0.0f, 8.0f, 188.4f, 8874.0f, 31.4f};
...
int16 ADRC_L(void)
{
    ...
    // ESO: 观测误差 = 估计速度 - 实际速度
    e = adrc_L.z1 - (float)real_speed_L;
    ...
    adrc_L.z1 += 0.01f * (adrc_L.z2 - adrc_L.beta1 * e + adrc_L.b0 * PID_outL);
    adrc_L.z2 += 0.01f * (-adrc_L.beta2 * e);
    
    // 线性控制律: u0 = kp * (r - z1)
    u0 = adrc_L.kp * ((float)target_speed_L - adrc_L.z1);
    
    // 扰动补偿 + 控制增益归一化: u = (u0 - z2) / b0
    // 结果存入 PID_outL（历史变量名，语义为"速度环控制器总输出"，供调试打印与下次 ESO 反馈）
    PID_outL = (u0 - adrc_L.z2) / adrc_L.b0;
...
int16 ADRC_R(void)
{
    ...
    e = adrc_R.z1 - (float)real_speed_R;
    
    // ESO 反馈同样复用 PID_outR（历史变量名，实际存储上一周期最终 PWM 输出）
    adrc_R.z1 += 0.01f * (adrc_R.z2 - adrc_R.beta1 * e + adrc_R.b0 * PID_outR);
    adrc_R.z2 += 0.01f * (-adrc_R.beta2 * e);
    
    u0 = adrc_R.kp * ((float)target_speed_R - adrc_R.z1);
    
    // 结果存入 PID_outR（历史变量名，语义同上）
    PID_outR = (u0 - adrc_R.z2) / adrc_R.b0;

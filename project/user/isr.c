...
// 函数名: TM3_IRQHandler
// 功能: 定时器3中断服务函数（本项目绑定为5ms循迹控制中断）
// 说明: 清除定时器3中断标志，若已注册回调函数则执行回调。
//       在main.c中通过 tim3_irq_handler = pit_track 绑定到循迹控制函数。
...
// 函数名: TM4_IRQHandler
// 功能: 定时器4中断服务函数（本项目绑定为2ms速度环中断）
// 说明: 清除定时器4中断标志，若已注册回调函数则执行回调。
//       在main.c中通过 tim4_irq_handler = pit_speed 绑定到速度环控制函数。
